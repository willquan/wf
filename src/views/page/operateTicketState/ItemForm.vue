...
<Row type="flex" justify="center" style="margin-bottom: 8px;">
    <Col span="20" :lg="15">
        <Card>
            <p slot="title">操作票状态信息</p>
            <a href="#" v-if="isEditable" slot="extra" @click.prevent="$refs.wfForm.resetFields()">
                <Icon type="ios-loop-strong"></Icon>
                清空表单
            </a>
            <Form ref="wfForm" :model="form" :rules="rules" label-position="left" :label-width="120">
                <FormItem prop="name" label="状态名称">
                    <Input v-model="form.name" :maxlength="10" placeholder="请输入操作票状态名称" :disabled="!isEditable"  class="check-input"></Input>
                </FormItem>
